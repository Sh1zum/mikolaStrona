<div class="ec_admin_list_line_item_fullwidth ec_admin_demo_data_line">
    ...
    <div class="ec_admin_settings_label"><div class="dashicons-before dashicons-admin-users"></div><span><?php _e( 'Language Settings', 'wp-easycart' ); ?></span></div>
    <div class="ec_admin_settings_input ec_admin_settings_live_payment_section">
   		
        <span><?php _e( 'Language Section', 'wp-easycart' ); ?></span>
        ...
        <form method="post" action="admin.php?page=wp-easycart-settings&subpage=language-editor&ec_action=add-new-language" name="wpeasycart_admin_form" id="wpeasycart_admin_form_lang2" novalidate="novalidate">
            <div><?php _e( 'Add Language', 'wp-easycart' ); ?>:<select name="ec_option_add_language" id="ec_option_add_language">
                <?php 
                $add_count = 0;
                ...
                <?php if( $add_count > 0 ){ ?>
                <div class="ec_admin_settings_input"><input type="submit" value="<?php _e( 'Add', 'wp-easycart' ); ?>" /></div>
                <?php }?>
            </div>
        </form>
        ...
        <div class="ec_admin_settings_input">
            <input type="submit" class="ec_admin_settings_simple_button" onclick="return ec_admin_save_language_editor( );" value="<?php _e( 'Save Options', 'wp-easycart' ); ?>" />
        </div>
    </div>
</div>