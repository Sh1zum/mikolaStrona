...
if( current_user_can( 'manage_options' ) ){
	...
	if( isset( $orderidarray ) ){	
		...
		$ids = implode( ',', $ids );
		$sql = "SELECT ec_product.*, GROUP_CONCAT( ec_categoryitem.category_id SEPARATOR ',' ) AS categories FROM ec_product LEFT JOIN ec_categoryitem ON ec_categoryitem.product_id = ec_product.product_id WHERE ec_product.product_id IN (".$ids.") GROUP BY ec_product.product_id";
		...
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT ec_product.*, GROUP_CONCAT( ec_categoryitem.category_id SEPARATOR ',' ) AS categories FROM ec_product LEFT JOIN ec_categoryitem ON ec_categoryitem.product_id = ec_product.product_id GROUP BY ec_product.product_id ORDER BY ec_product.product_id ASC LIMIT %d, 500", ( $setnum-1 )*500 ), ARRAY_A );
