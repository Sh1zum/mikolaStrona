...
function wpeasycart_verify_subscription_length( ){
    if( jQuery( document.getElementById( 'subscription_bill_period' ) ).val( ) == 'Y' && Number( jQuery( document.getElementById( 'subscription_bill_length' ) ).val( ) ) > 1 ){
        ...
        alert( 'Subscription with yearly interval has a max 1 year billing interval.' );
        ...
        alert( 'Subscription with monthly interval has a max 12 month billing interval.' );
    }
}
...
function ec_admin_save_product_details_googlemerchant( ){
	jQuery( document.getElementById( "ec_admin_product_details_google_merchant_loader" ) ).fadeIn( 'fast' );
	
	var data = {
		action: 'ec_admin_ajax_save_product_details_google_merchant',
		product_id: ec_admin_get_value( 'product_id', 'hidden' ),
    ...
    jQuery.ajax({url: ajax_object.ajax_url, type: 'post', data: data, success: function(data){
        ...
        for( var i=0; i<prod_columns.length; i++ ){
            ...
            if( json_data.activate_in_store == '1' ){
                ...
                jQuery( prod_columns[6] ).find( '.deactivate-product' ).html( '<a href="admin.php?&page=wp-easycart-products&subpage=products&product_id=' + json_data.product_id + '&ec_admin_form_action=deactivate-product" aria-label="Deactivate" title="Deactivate"><div class="dashicons-before dashicons-hidden"></div></a>' );
            }else{
                jQuery( prod_columns[4] ).html( 'No' );
                jQuery( prod_columns[6] ).find( '.deactivate-product' ).html( '<a href="admin.php?&page=wp-easycart-products&subpage=products&product_id=' + json_data.product_id + '&ec_admin_form_action=deactivate-product" aria-label="Activate" title="Activate"><div class="dashicons-before dashicons-visibility"></div></a>' );
            }
        }
        if( goto_edit ){
