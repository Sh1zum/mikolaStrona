...
final class wp_easycart_admin_payments{
	...
	public function load_free_paypal_credit_field( ){
		echo '<div style="font-weight:bold; margin:15px 0 0;">' . __( 'Add PayPal Express', 'wp-easycart' ) . '</div>';
		echo '<div>' . __( 'Enable PayPal Express', 'wp-easycart' ) . ' <span class="dashicons dashicons-lock" style="color:#FC0; float:left; margin-top:5px;"></span><select onchange="show_pro_required( ); return false;">';
        echo '<option value="0"';
		if( get_option( 'ec_option_paypal_enable_pay_now' ) == '0' ) 
			echo ' selected';
		echo '>' . __( 'Keep PayPal Standard, Redirect Users to PayPal for Payment', 'wp-easycart' ) . '</option>';
        echo '<option value="1"';
		if( get_option( 'ec_option_paypal_enable_pay_now' ) == '1' )
			echo ' selected';
		echo '>' . __( 'YES! Enable PayPal Express and Keep Customers on My Site', 'wp-easycart' ) . '</option>';
        echo '</select></div>';
		
		echo '<div>' . __( 'Advertise PayPal Credit', 'wp-easycart' ) . ' <span class="dashicons dashicons-lock" style="color:#FC0; float:left; margin-top:5px;"></span><select onchange="show_pro_required( ); return false;">';
        echo '<option value="0"';
		if( get_option( 'ec_option_paypal_enable_credit' ) == '0' ) 
			echo ' selected';
		echo '>' . __( 'Do Not Advertise PayPal Credit', 'wp-easycart' ) . '</option>';
        echo '<option value="1"';
		if( get_option( 'ec_option_paypal_enable_credit' ) == '1' )
			echo ' selected';
		echo '>' . __( 'Advertise PayPal Credit', 'wp-easycart' ) . '</option>';
        echo '</select></div>';
	}
	...
	public function save_stripe_connect( ){
		...
		update_option( 'ec_option_stripe_company_country', $_POST['ec_option_stripe_company_country'] );
		update_option( 'ec_option_stripe_enable_apple_pay', $_POST['ec_option_stripe_enable_apple_pay'] );
	...
	public function process_square_app( ){
		...
		// Handle a Successful Connect Attempt
            ...
            $refresh_token = preg_replace( "/[^A-Za-z0-9 \-\._\~\+\/]/", '', $_GET['refresh_token'] );
			...
			if( isset( $_GET['sandbox'] ) ){
                ...
                $response = file_get_contents( "https://connect.wpeasycart.com/square-sandbox/disconnect.php?access_token=" . $access_token );
                ...
                $response = file_get_contents( "https://connect.wpeasycart.com/square/disconnect.php?access_token=" . $access_token );
