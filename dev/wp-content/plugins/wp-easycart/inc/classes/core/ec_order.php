		...
		if($this->order_id != 0){
			...
			if( $gift_card_processed_successfully ){
				...
				if( $this->process_result == "1" ){
					...
					// Deconetwork if used
					if( ( $payment_type != "third_party" && !$this->payment->is_3d_auth && $payment_type != "ideal" ) || $payment_type == "affirm" || ( $payment_type == "third_party" && ( get_option( 'ec_option_payment_process_method' ) == "stripe" || get_option( 'ec_option_payment_process_method' ) == "stripe_connect" ) ) )
	...
	public function process_deconetwork_complete( $is_paid ){
		...
		if( get_option( 'ec_option_deconetwork_url' ) != "" ){
			...
			if( count( $deconetwork_ids ) > 0 ){
                ...
                $this->mysqli->insert_response( $this->order_id, 0, "DecoNetwork Response", $response );
