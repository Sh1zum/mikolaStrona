...
class ec_db_manager{
    ...
    public function verify_db( ){
        ...
        foreach( $create_statements as $create_statement ){
            preg_match_all( '/CREATE\sTABLE\s([a-z\_]*)\s\((.*)\)/s', $create_statement, $table_data );
                    ...
                    if( !$table_schema_rows ){
                        ...
                        foreach( $table_rows as $table_row_item ){
                            ...
                            // Exists, lets check its valid
                                ...
                                }else if( trim( $table_schema[ $table_row_item->Field ]->Default ) != trim( $table_row_item->Default ) ){
                                    //$errors[] = array( 'type' => 'incorrect_default', 'sql_fix' => 'ALTER TABLE ' . $table_name . ' MODIFY COLUMN ' . $table_row_item->SQL, 'error' => $table_name . '.' . $table_row_item->Field . ' is the wrong "default", listed as "' . trim( $table_schema[ $table_row_item->Field ]->Default ) . '" should be "' . trim( $table_row_item->Default ) . '"' );
