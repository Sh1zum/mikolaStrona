...
class ec_product{
	...
	public function display_price( ){
		...
		$output = apply_filters( 'wp_easycart_product_details_price_pre_num', $output, $this->product_id );
		...
		$output = apply_filters( 'wp_easycart_product_details_price_post_num', $output, $this->product_id );
	...
	public function display_product_pricing_no_vat( ){
		...
		$tax_price = new ec_tax( $price, $price, $price, "", "", false, 0, (object) array( 
            'cart' => array( 
                (object) array( 
		...
		$tax_list_price = new ec_tax( $list_price, $list_price, $list_price, "", "", false, 0, (object) array( 
            'cart' => array( 
                (object) array( 
                    'product_id' => $this->product_id, 
                    'total_price' => $this->price, 
	...
	public function display_product_pricing_vat( ){
		...
		$output = "";
		if( $this->list_price != "0.000" )
			$output .= "<span class=\"ec_product_old_price\">" . $GLOBALS['currency']->get_currency_display( $list_price ) . "</span><span class=\"ec_product_sale_price ec_product_sale_price_" . $this->product_id . "\">" . $GLOBALS['currency']->get_currency_display( $price ) . "</span> " . $GLOBALS['language']->get_text( 'product_details', 'product_details_vat_included' ) . " " . number_format( $tax_price->vat_rate, 0, '', '' ) . "% " . $GLOBALS['language']->get_text( 'cart_totals', 'cart_totals_vat' );
		else
			$output .= "<span class=\"ec_product_price ec_product_price_" . $this->product_id . "\">" . $GLOBALS['currency']->get_currency_display( $price ) . "</span> " . $GLOBALS['language']->get_text( 'product_details', 'product_details_vat_included' ) . " " . number_format( $tax_price->vat_rate, 0, '', '' ) . "% " . $GLOBALS['language']->get_text( 'cart_totals', 'cart_totals_vat' );
