...
class ec_accountpage{
	...
	public function display_account_error( $error_code = '' ){
        ...
        if( isset( $_GET['account_error'] ) && in_array( $_GET['account_error'], $valid_error_codes ) ){
			...
			$error_text = $GLOBALS['language']->get_text( "ec_errors", $error_code );
	...
	public function display_account_success( $success_code = '' ){
        ...
        if( isset( $_GET['account_success'] ) && in_array( $_GET['account_success'], $valid_success_codes ) ){
			...
			$success_text = $GLOBALS['language']->get_text( "ec_success", $success_code );
		...
		if( isset( $_POST['ec_account_register_email'] ) && isset( $_POST['ec_account_register_password'] ) && $_POST['ec_account_register_email'] != "" && $_POST['ec_account_register_password'] != "" ){
			...
			if( $recaptcha_valid ){
				...
				if( apply_filters( 'wp_easycart_sync_wordpress_users', false ) ){
					...
					$wp_user_id = wp_create_user( $user_name, $_POST['ec_account_register_password'], $email );
					add_user_meta( $wp_user_id, 'wpeasycart_user_id', $user_id, true );
				}
				
				do_action( 'wpeasycart_account_added', $user_id, $email, $_POST['ec_account_register_password'] );
	...
	private function process_update_password( ){
		...
		if( apply_filters( 'wpeasycart_custom_verify_new_password', false, $_POST['ec_account_password_new_password'] ) ){
			...
			if( apply_filters( 'wp_easycart_sync_wordpress_users', false ) ){
				$wp_user = get_user_by( 'email', $GLOBALS['ec_user']->email );
				if( $wp_user ){
					wp_set_password( $_POST['ec_account_password_retype_new_password'], $wp_user->ID );
				}
			}
	...
	private function process_order_create_account( ){
		...
		if( $this->mysqli->does_user_exist( $email ) ){
			...
			do_action( 'wpeasycart_account_added', $user_id, $email, $_POST['ec_password'] );
	...
	public function display_subscription_update_form_end( ){
		echo "<input type=\"hidden\" name=\"stripe_subscription_id\" id=\"stripe_subscription_id\" value=\"" . $this->subscription->get_stripe_id( ) . "\" />";
		echo "<input type=\"hidden\" name=\"subscription_id\" id=\"subscription_id\" value=\"" . $this->subscription->subscription_id . "\" />";
