...
class ec_moneris_us extends ec_gateway{
	...
	function get_gateway_response( $gateway_url, $gateway_data, $gateway_headers ){
		...
		$store_id		= get_option( 'ec_option_moneris_us_store_id' );
		$api_token		= get_option( 'ec_option_moneris_us_api_token' );
	...
	function handle_gateway_response( $response ){
		...
		if( (int) $response->getResponseCode( ) < 50 )
			$this->is_success = true;
		else
			$this->is_success = false;
		
		$this->mysqli->insert_response( $this->order_id, !$this->is_success, "Moneris US", print_r( $response, true ) );
