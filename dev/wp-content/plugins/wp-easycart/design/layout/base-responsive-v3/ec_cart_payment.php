...
?>
<?php if( ( isset( $_GET['ideal'] ) && isset( $_GET['client_secret'] ) && isset( $_GET['source'] ) && $_GET['ideal'] == 'returning' ) || ( isset( $ideal_source ) && isset( $ideal_client_secret ) && $ideal_source != '' && $ideal_client_secret != '' ) ){ ?>
<style>
...
<script>
	...
	var ec_ideal_tries = 0;
	function ec_ideal_waiting_for_response( ){
        if( ec_ideal_tries >= 3 ){ // Skip checking for order update, probably bad webhook or network issues.
            clearInterval( ec_ideal_timer );
            var data = {
                action: 'ec_ajax_check_stripe_ideal_order_skip',
                source: '<?php echo htmlspecialchars( ( ( isset( $_GET['source'] ) ) ? $_GET['source'] : $ideal_source ), ENT_QUOTES ); ?>',
                client_secret: '<?php echo htmlspecialchars( ( ( isset( $_GET['client_secret'] ) ) ? $_GET['client_secret'] : $ideal_client_secret ), ENT_QUOTES ); ?>'
            };
            jQuery.ajax({url: wpeasycart_ajax_object.ajax_url, type: 'post', data: data, success: function(data){ 
                ...
                }else if( json_response.is_approved == '1' ){
                    window.location.href = '<?php echo $this->cart_page . $this->permalink_divider . "ec_page=checkout_success&order_id=" ?>' + json_response.order_id;
                }else{
                    window.location.href = '<?php echo $this->cart_page . $this->permalink_divider . "ec_page=checkout_success&ec_cart_error=ideal-pending&order_id=" ?>' + json_response.order_id;
                }
            } } );
        }else{
            var data = {
...
<div class="ec_cart_left" id="ec_cart_payment_one_column" style="margin-bottom:25px;">
    ...
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_first_name, ENT_QUOTES ); ?> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_last_name, ENT_QUOTES ); ?>
    </div>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->billing_company_name ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_company_name, ENT_QUOTES ); ?>
    </div>
    <?php }?>

    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_1, ENT_QUOTES ); ?>
    </div>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_2 ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_2, ENT_QUOTES ); ?>
    </div>
    <?php }?>

    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_city, ENT_QUOTES ); ?>, <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_state, ENT_QUOTES ); ?> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_zip, ENT_QUOTES ); ?>
    </div>

    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_country_name, ENT_QUOTES ); ?>
    </div>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->billing_phone ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_phone, ENT_QUOTES ); ?>
    </div>
    <?php }?>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->vat_registration_number ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <strong><?php echo $GLOBALS['language']->get_text( 'cart_billing_information', 'cart_billing_information_vat_registration_number' ); ?>:</strong> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->vat_registration_number, ENT_QUOTES ); ?>
    </div>
    <?php }?>
    ...
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_first_name, ENT_QUOTES ); ?> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_last_name, ENT_QUOTES ); ?>
    </div>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->shipping_company_name ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_company_name, ENT_QUOTES ); ?>
    </div>
    <?php }?>

    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_1, ENT_QUOTES ); ?>
    </div>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_2 ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_2, ENT_QUOTES ); ?>
    </div>
    <?php }?>

    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_city, ENT_QUOTES ); ?>, <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_state, ENT_QUOTES ); ?> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_zip, ENT_QUOTES ); ?>
    </div>

    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_country_name, ENT_QUOTES ); ?>
    </div>

    <?php if( strlen( $GLOBALS['ec_cart_data']->cart_data->shipping_phone ) > 0 ){ ?>
    <div class="ec_cart_input_row">
        <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_phone, ENT_QUOTES ); ?>
    </div>
    <?php }?>
...
<div class="ec_cart_left">
	...
	<?php if( get_option( 'ec_option_payment_process_method' ) == 'square' ){ $this->print_square_payment_button( true ); } ?>
    	...
    	<div class="ec_cart_box_section ec_affirm_box">
        	<script>
				...
				affirm.checkout({
					...
					billing: {
						name: {
							first:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_first_name, ENT_QUOTES ); ?>",
							last:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_last_name, ENT_QUOTES ); ?>"
						},
						address: {
							line1:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_1, ENT_QUOTES ); ?>",
							line2:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_2, ENT_QUOTES ); ?>",
							city:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_city, ENT_QUOTES ); ?>",
							state:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_state, ENT_QUOTES ); ?>",
							zipcode:				"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_zip, ENT_QUOTES ); ?>",
							country:				"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_country, ENT_QUOTES ); ?>"
						},
						phone_number:				"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_phone, ENT_QUOTES ); ?>"<?php if( !class_exists( 'Email_Encoder' ) && !function_exists( 'eae_encode_emails' ) ){ ?>,
						email:						"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->email, ENT_QUOTES ); ?>"<?php }?>
					},
					shipping: {
						name: {
							first:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_first_name, ENT_QUOTES ); ?>",
							last:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_last_name, ENT_QUOTES ); ?>"
						},
						address: {
							line1:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_1, ENT_QUOTES ); ?>",
							line2:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_2, ENT_QUOTES ); ?>",
							city:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_city, ENT_QUOTES ); ?>",
							state:					"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_state, ENT_QUOTES ); ?>",
							zipcode:				"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_zip, ENT_QUOTES ); ?>",
							country:				"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_country, ENT_QUOTES ); ?>"
						},
						phone_number:				"<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_phone, ENT_QUOTES ); ?>"
					},
    ...
    <script>
	var applicationId = '<?php if( get_option( 'ec_option_square_application_id' ) != '' ){ echo get_option( 'ec_option_square_application_id' ); }else{ echo ( get_option( 'ec_option_square_is_sandbox' ) ) ? 'sandbox-sq0idb-khAAob2bNi889KPQSVsF6Q' : 'sq0idp-H8Mnz1zzbv1mOyeWyKpF6Q'; } ?>';
	var given_name = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_first_name, ENT_QUOTES ); ?>';
	var family_name = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_last_name, ENT_QUOTES ); ?>';
	var address1 = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_1, ENT_QUOTES ); ?>';
	var address2 = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_2, ENT_QUOTES ); ?>';
	var addressLines = Array( address1 );
	if( address2.length > 0 ){
		addressLines.push( address2 );
	}
	var city = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_city, ENT_QUOTES ); ?>';
	var state = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_state, ENT_QUOTES ); ?>';
	var zip = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_zip, ENT_QUOTES ); ?>';
	var country = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_country, ENT_QUOTES ); ?>';
	var phone = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_phone, ENT_QUOTES ); ?>';<?php if( !class_exists( 'Email_Encoder' ) && !function_exists( 'eae_encode_emails' ) ){ ?>
	var email = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->email, ENT_QUOTES ); ?>';<?php }?>
	var billing_contact = {
	...
	const verificationDetails = {
		amount: '<?php echo number_format( $this->order_totals->grand_total, 2, '.', '' ); ?>',
		intent: 'CHARGE',
	...
	try {
		var paymentForm = new SqPaymentForm({
            ...
            autoBuild: <?php echo ( ( get_option( 'ec_option_cache_prevent' ) ) ? 'false' : 'true' ); ?>,
			inputClass: 'sq-input',
			inputStyles: [{
				fontSize: '15px'
			}],
			...
			locationId: '<?php $location_id = ( get_option( 'ec_option_square_is_sandbox' ) ) ? get_option( 'ec_option_square_sandbox_location_id' ) : get_option( 'ec_option_square_location_id' ); if( $location_id == '' ){ $square = new ec_square( ); $location_id = $square->get_location_id( ); } echo $location_id; ?>',
			callbacks: {
				...
				paymentFormLoaded: function() {
					paymentForm.setPostalCode('<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_zip ); ?>');
				}
			}
		} );
	}catch( err ){
    ...
    <div id="ec_credit_card_form"<?php if( $this->get_selected_payment_method( ) == "credit_card" ){ ?> style="display:block;"<?php }?>>
    	<div class="ec_cart_box_section">
            ...
            <?php if( $this->order_totals->grand_total >= .5 ){ ?>
            ...
            <script><?php
				...
				?>
				<?php $stripe_payment_intent_client_secret = $this->get_stripe_intent_client_secret( ); ?>
				...
				try {
					...
					form.addEventListener( 'submit', function( event ){
						...
						if( payment_method != 'credit_card' ){
							...
							var name = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_first_name, ENT_QUOTES ); ?> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_last_name, ENT_QUOTES ); ?>';
							var address1 = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_1, ENT_QUOTES ); ?>';
							var address2 = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_address_line_2, ENT_QUOTES ); ?>';
							var city = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_city, ENT_QUOTES ); ?>';
							var state = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_state, ENT_QUOTES ); ?>';
							var zip = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_zip, ENT_QUOTES ); ?>';
							var country = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_country, ENT_QUOTES ); ?>';
							var phone = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_phone, ENT_QUOTES ); ?>';
							var shipping_name = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_first_name, ENT_QUOTES ); ?> <?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_last_name, ENT_QUOTES ); ?>';
							var shipping_address1 = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_1, ENT_QUOTES ); ?>';
							var shipping_address2 = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_address_line_2, ENT_QUOTES ); ?>';
							var shipping_city = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_city, ENT_QUOTES ); ?>';
							var shipping_state = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_state, ENT_QUOTES ); ?>';
							var shipping_zip = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_zip, ENT_QUOTES ); ?>';
							var shipping_country = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_country, ENT_QUOTES ); ?>';
							var shipping_phone = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->shipping_phone, ENT_QUOTES ); ?>';<?php if( !class_exists( 'Email_Encoder' ) && !function_exists( 'eae_encode_emails' ) ){ ?>
							var email = '<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->email, ENT_QUOTES ); ?>';<?php }?>
            ...
            <?php } ?>
			<div class="ec_cart_input_row">
				<div<?php if( get_option( 'ec_option_payment_process_method' ) == "square" ){ ?> id="sq-card-number"<?php }?>>
           	...
           	<div class="ec_cart_input_row">
			    ...
			    <div class="ec_cart_input_right_half ec_cart_input_right_half_third">
                	<div id="sq-postal-code">
                    	<input name="sq-postal-code" type="text" class="input-lg form-control cc-cvc" autocomplete="off" value="<?php echo htmlspecialchars( $GLOBALS['ec_cart_data']->cart_data->billing_zip ); ?>" readonly="readonly" />
            		</div>
...
<?php if( $this->use_payment_gateway( ) && get_option( 'ec_option_payment_process_method' ) == "square" && get_option( 'ec_option_cache_prevent' ) ){ ?>
<script type="text/javascript">
    paymentForm.build( );
</script>
<?php }else if( $this->use_payment_gateway( ) && get_option( 'ec_option_payment_process_method' ) != "square" && get_option( 'ec_option_cache_prevent' ) ){ ?>
<script type="text/javascript">
